...
def plot_switch_training() -> None:
    data = load_switch_training()
    panels = (
        ("echo_to_rl", "ECHO (SFT) → RL", SFT_COLOR, [(0, 100, "ECHO (SFT)"), (100, 200, "RL")]),
        ("rl_to_echo", "RL → ECHO (SFT)", RL_COLOR, [(0, 100, "RL"), (100, 200, "ECHO (SFT)")]),
    )
    figure, axes = plt.subplots(1, 2, figsize=(13.2, 4.7), sharey=True)
    figure.subplots_adjust(top=0.84, bottom=0.13, left=0.08, right=0.98, wspace=0.18)
    for axis, (key, title, color, phases) in zip(axes, panels, strict=True):
        steps, rewards = data[key]
        axis.plot(steps, rewards, color=color, linewidth=0.9, alpha=0.18)
        axis.plot(steps, moving_mean(rewards), color=color, linewidth=2.35)
        add_phases(axis, phases)
        style_axis(axis)
        axis.set_title(title, loc="left", color=TEXT, fontsize=13, weight="bold")
    axes[0].set_ylabel("Mean progress reward", color=TEXT, fontsize=10)
    save_figure(figure, "sft_switch_training")


def plot_switch_eval() -> None:
    data = load_switch_eval()
    panels = (
        (
        ...
        (
            ...
            RL_COLOR,
            "o",
            [(0, 100, "RL"), (100, 200, "ECHO (SFT)")],
        ),
    )
    figure, axes = plt.subplots(1, 2, figsize=(13.2, 4.7), sharey=True)
    figure.subplots_adjust(top=0.84, bottom=0.13, left=0.08, right=0.98, wspace=0.18)
    for axis, (key, title, color, marker, phases) in zip(axes, panels, strict=True):
        steps, means, deviations = data[key]
        axis.fill_between(
            steps,
            np.clip(means - deviations, 0, 1),
            np.clip(means + deviations, 0, 1),
            color=color,
            alpha=0.12,
            linewidth=0,
        ...
        style_axis(axis)
        axis.set_title(title, loc="left", color=TEXT, fontsize=13, weight="bold")
    axes[0].set_ylabel("Held-out reward", color=TEXT, fontsize=10)
    save_figure(figure, "sft_switch_eval")


def plot_switch_eval_overlay() -> None:
    data = load_switch_eval()
    series = (
        ("echo_sft100_to_rl100", "ECHO (SFT) → RL", SFT_COLOR, "D", "-"),
        ("rl100_to_echo_sft100", "RL → ECHO (SFT)", RL_COLOR, "o", "--"),
    )
    figure, axis = plt.subplots(figsize=(10.8, 4.7))
    figure.subplots_adjust(top=0.80, bottom=0.14, left=0.09, right=0.98)
    ...
    style_axis(axis)
    axis.axvline(100, color=SWITCH, linestyle="--", linewidth=1.15, zorder=1)
    axis.text(
        103,
        0.97,
        "Switch",
    ...
    axis.set_title("Held-out evaluation reward", loc="left", color=TEXT, fontsize=13, weight="bold")
    axis.set_ylabel("Held-out reward", color=TEXT, fontsize=10)
    ...
    save_figure(figure, "sft_switch_eval_overlay")
...
def main() -> None:
    plot_switch_training()
    plot_switch_eval()
    plot_switch_eval_overlay()
    plot_four_phase()
